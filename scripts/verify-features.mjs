...
try {
  ...
  // Score badges sit on the green/red accents — text must stay readable (WCAG AA
  // for bold text is 3:1; we require a comfortable >= 4.5:1 across every badge).
  const srgb = (c) => {
    const x = c / 255
  ...
  const badgeColors = await page.locator('.fg-pill', { hasText: '%' }).evaluateAll((els) =>
    els.map((el) => {
  ...
  let worst = Infinity
  for (const { color, bg } of badgeColors) worst = Math.min(worst, contrast(color, bg))
  if (worst < 4.5) fail(`score badge text contrast too low: ${worst.toFixed(2)}:1 (need >= 4.5)`)
  else log(`✓ score badge text is legible on green/red (min contrast ${worst.toFixed(2)}:1)`)
